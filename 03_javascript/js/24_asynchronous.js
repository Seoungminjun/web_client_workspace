...
document.querySelector("#btn2").addEventListener('click' , () =>{
    loadScript('js/24_test.js', () => {
        test(); // test.js 안의 test함수  Uncaught ReferenceError: test is not defined
});
    
});

const loadScript = (src, callback) => {
    // js/test.js를 동적으로 로드하고, 그안의 함수 test를 호출 
    const script = document.createElement('script'); // <script></script>
    script.src = src; // <script src="js/24_asynchronous.js"></script> script 만들어짐
