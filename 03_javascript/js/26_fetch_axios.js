 ...
 * - {"id":"shqkel","company":"KH정보교육원","classroom":"352","cnt":78}
 * - 속성명은 반드시 쌍따옴표로 감싼다.
 * - 문자열은 반드시 쌍따옴표로 감싼다.
 * - 객체/배열 사용가능
 * - 숫자/boolean/null은 그대로작성
 * 
 */
document.querySelector("#btn1").addEventListener('click', () => {
    ...
    fetch(url)
    .then((response) => {
        // fetch의 첫번째 콜백함수는 응답이 시작되는 순간 호출 -> 응답데이터 확인 불가 
...
document.querySelector("#btn2").addEventListener('click', () => {
    const url ='https://api.github.com/users/Seoungminjun';
    fetch(url)
...
document.querySelector("#btn5").addEventListener('click', () =>{
    ...
    axios(url,{
        headers : {
            'x-api-key' : 'live_JpqKZ5gyG7yps04hSvRjZXUvoKjW6CX9Ld7LvezCZpeWq6BLIPMtY1wZgFNW7Rfr'
        },
        params : {
            limit : 1 //기본값
        }
